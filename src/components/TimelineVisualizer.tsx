      ...
      <div className="relative pl-4 sm:pl-6 border-l-2 border-slate-800 space-y-4">
        {timeline.map((segment, idx) => (
          <div key={idx} className="relative group">
            ...
            {/* Storyboard Card */}
            <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800/90 hover:border-cyan-500/40 transition-all space-y-2.5">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-md bg-amber-950/80 text-amber-300 border border-amber-500/40 font-mono text-xs font-bold flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    {segment.timeframe}
                  </span>
                  {segment.camera && (
                    <span className="px-2 py-0.5 rounded-md bg-cyan-950/80 text-cyan-300 border border-cyan-500/40 font-mono text-xs flex items-center gap-1 max-w-md truncate" title={segment.camera}>
                      <Camera className="w-3 h-3 shrink-0" />
                      <span className="truncate">{segment.camera}</span>
                    </span>
                  )}
                </div>
              ...
              {/* Audio & Dialogue */}
              {segment.audio && (
                <div className="flex items-start gap-1.5 pt-1 text-xs text-emerald-300/90 bg-emerald-950/30 px-2.5 py-1.5 rounded-lg border border-emerald-500/20 font-mono">
                  <Volume2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                  <span className="leading-relaxed">{segment.audio}</span>
                </div>
              )}
            </div>
          </div>
